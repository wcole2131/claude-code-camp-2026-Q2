...
def test_add_message_accepts_a_list_of_content_blocks():
    ctx = Context(task=None)
    blocks = [{"type": "tool_use", "id": "toolu_01X", "name": "move", "input": {"direction": "north"}}]

    ctx.add_message("assistant", blocks)

    assert ctx.messages[0].content == blocks
