...
class Tui(App[None]):
    ...
    def _handle_event(self, event: dict[str, Any]) -> None:
        ...
        if phase == "iteration":
            ...
            self._write_line(f"[context compacted — {dropped} messages dropped to free space]")
    ...
    def _render_progress(self) -> None:
        ...
        if self._live["active"]:
            ...
            used = self._fmt_tokens(self._repl.context.current_tokens)
            max_tokens = self._fmt_tokens(self._repl.context.context_window)
            text = Text(
                f"  [ready]   ctx {used} / {max_tokens} ({pct}%)   {self._turn_count} turns",
                style=Style(color=ANSI_COLORS[color]),
            )
    ...
    def _render_status(self) -> None:
        ...
        ctx_indicator = " ⚠ " if pct >= self.CTX_ALERT_PCT else " "
        bar = f" boukensha v{ver} · {model}  ·  ctx {used}/{max_tokens} ({pct}%){ctx_indicator}·  {tools} tools  ·  {clock} "
        status.update(Text(bar.ljust(width), style=Style(color=ANSI_COLORS["white"], bgcolor=ANSI_COLORS["bright_black"])))

    @classmethod
    def _ctx_color(cls, pct: int) -> str:
        if pct >= cls.CTX_ALERT_PCT:
            return "red"
        if pct >= cls.CTX_WARN_PCT:
            return "yellow"
        return "bright_black"
