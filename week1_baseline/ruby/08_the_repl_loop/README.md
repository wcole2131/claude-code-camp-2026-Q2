...
We are not fixing these now to preserve future layers but making not of thins we 
observed that might need fixing.

- We need to determine if quite and loud are legacy logging or if they actually provide
  detail logs.
- It looks like REPL loop we will initialize on every turn an agent.  It seems like we
