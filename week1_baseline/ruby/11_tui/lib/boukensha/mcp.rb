...
module Boukensha
  ...
  class MCP
    ...
    def connect!
      # Unset any inherited Bundler/rbenv env (BUNDLE_GEMFILE, RUBYOPT's
      # -rbundler/setup, RBENV_VERSION/RBENV_DIR, etc.) before spawning.
      # Without this, a server spawned via `bundle exec` from *inside*
      # another `bundle exec` process (e.g. the ruby step's own agent
      # launching mud_manager_mcp) inherits the parent's BUNDLE_GEMFILE or
      # rbenv version pin and resolves against the wrong Gemfile/Ruby
      # instead of doing a fresh cwd-based lookup in `dir:`. This matters
      # even when the parent wasn't run through `bundle exec` -- an rbenv
      # shim exports RBENV_VERSION into its own process's env once it picks
      # a version, and that leaks to children the same way.
      #
      ...
      full_env = ENV.to_h
      full_env.each_key { |k| full_env[k] = nil if k.start_with?("BUNDLE_") || k.start_with?("RBENV_") || k == "RUBYOPT" }
      ...
      # the shim's cwd-based lookup in `dir:`. Strip it back to the shims
      # dir so a server pinned to a different Ruby version resolves its own.
      if rbenv_root
        full_env["PATH"] = full_env["PATH"].split(File::PATH_SEPARATOR)
          .reject { |p| p.start_with?("#{rbenv_root}/versions/") }
