...
#
# Carried over unchanged from step 10 -- it doesn't exercise the TUI (the
# TUI is interactive and is run via the global `boukensha` executable
# instead, see README.md). Demonstrates connecting to mud_manager_mcp
# (week1_baseline/mud_manager_mcp), a real MCP server, via Boukensha's
# default mcp_servers: wiring -- no MUD tool implementation lives in this
# framework itself (see docs/plans/mud_manager/mcp_mud_plan.md). Connection
# credentials come from ~/.boukensha/settings.yaml (mud:
# host/port/username/password) by default. Set BOUKENSHA_DIR to point at a
