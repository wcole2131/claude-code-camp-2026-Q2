...
# MudTools registers MUD-gameplay tools against a Boukensha::Registry.
#
# This used to live inside the Boukensha framework itself, as
# Boukensha::Tools::Mud (week1_baseline/ruby/10_standard_tool_library/lib/
# boukensha/tools/mud.rb) -- it moved here because the framework no longer
# ships any domain tool implementations at all (see
# docs/plans/mud_manager/mcp_mud_plan.md). This is now server-side code,
# owned by mud_manager_mcp, not agent-side code.
#
# A single MudManager::Session is created when the tools are registered and
# shared by every tool via closure — the server logs in once and reuses the
...
module MudTools
  def self.register(registry, host: "localhost", port: 4000, name:, password:)
    ...
    registry.tool "look",
      ...
      parameters: {
        target:      { type: "string", description: "Item, mob, or player name to inspect. Omit entirely to describe the current room.", required: false },
        preposition: { type: "string", description: "Preposition: in, at, north, east, south, west, up, down (optional)", required: false }
    ...
    registry.tool "attack",
      ...
      parameters: {
        ...
        style:  { type: "string", description: "Attack style: kill | hit | murder (default: kill)", required: false }
    ...
    registry.tool "say",
      ...
      parameters: {
        ...
        mode: { type: "string", description: "Mode: say | emote | reply (default: say)", required: false }
      } do |text:, mode: "say"|
      next guard.call if guard.call
      begin
    ...
    registry.tool "tell",
      ...
      parameters: {
        ...
        mode:   { type: "string", description: "Mode: tell | whisper | ask (default: tell)", required: false }
    ...
    registry.tool "get_item",
      ...
      parameters: {
        ...
        container: { type: "string",  description: "Container to get it from (optional)", required: false },
        count:     { type: "integer", description: "Number of items to get (optional)", required: false }
    ...
    registry.tool "drop_item",
      ...
      parameters: {
        ...
        mode:  { type: "string",  description: "Mode: drop | donate | junk (default: drop)", required: false },
        count: { type: "integer", description: "Number of items (optional)", required: false }
    ...
    registry.tool "equip_item",
      ...
      parameters: {
        ...
        body_loc: { type: "string", description: "Body location to wear on (optional, e.g. 'head', 'finger')", required: false }
    ...
    registry.tool "consume_item",
      ...
      parameters: {
        ...
        mode: { type: "string", description: "Mode: eat | drink | taste | sip (default: eat)", required: false }
      } do |item:, mode: "eat"|
      next guard.call if guard.call
      begin
    ...
    registry.tool "cast_spell",
      ...
      parameters: {
        ...
        target: { type: "string", description: "Target mob, player, or object (optional)", required: false }
      } do |spell:, target: nil|
      next guard.call if guard.call
      begin
    ...
    registry.tool "use_magic_item",
      ...
      parameters: {
        ...
        target_args: { type: "string", description: "Optional target arguments (e.g. mob name for a wand)", required: false }
    ...
    registry.tool "shop",
      ...
      parameters: {
        ...
        args:   { type: "string", description: "Item name or number (optional)", required: false }
      } do |action:, args: nil|
      next guard.call if guard.call
      begin
    ...
    registry.tool "practice",
      ...
      parameters: {
        skill: { type: "string", description: "Skill name to practice (omit to list all)", required: false }
      } do |skill: nil|
      next guard.call if guard.call
