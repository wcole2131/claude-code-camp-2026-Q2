...
lives in `.claude/agents/`, but the scripts, `data/`, `assets/`, and
`references/` directories are siblings at the top of the project, not next
to this file) and are self-contained bash (they auto-locate their own
directory, so call them by absolute or relative path from anywhere).
Credentials and connection settings default to this project's
dummy/helloworld character on localhost:4000, configurable via env vars in
`scripts/mud_env.sh` if that ever changes.
