...
MUD_SESSION="${MUD_SESSION:-tbamud}"
MUD_WAIT="${MUD_WAIT:-0.8}"
MUD_STATE_DIR="${MUD_STATE_DIR:-$DIR/.state}"

# Project root is always 4 levels above scripts/ for a project-local skill
# (scripts -> mud-player -> skills -> .claude -> project root).
MUD_PROJECT_ROOT="${MUD_PROJECT_ROOT:-$(cd "$DIR/../../../.." && pwd)}"
MUD_DATA_DIR="${MUD_DATA_DIR:-$MUD_PROJECT_ROOT/data}"
