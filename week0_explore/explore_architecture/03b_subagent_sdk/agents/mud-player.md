...
All scripts live in `scripts/` at the project root (this agent definition
lives in `agents/mud-player.md` and is loaded explicitly by
`scripts/agent.ts` via the Claude Agent SDK's `AgentDefinition`, rather than
being auto-discovered from `.claude/agents/`; the scripts, `data/`,
`assets/`, and `references/` directories are siblings at the top of the
project, not next to this file) and are self-contained bash (they
auto-locate their own directory, so call them by absolute or relative path
from anywhere). Credentials and connection settings default to this
project's dummy/helloworld character on localhost:4000, configurable via
env vars in `scripts/mud_env.sh` if that ever changes.
...
## Playing multiple characters concurrently

The tmux session name, read-offset state, and memory directory
(`data/<user>/`) are all namespaced by `MUD_USER`, so two separate agents
(or two separate terminal invocations) each exporting a different `MUD_USER`
get fully independent sessions that don't collide or overwrite each other's
memory. To drive `smarty` instead of the default `dummy`, export both
credentials before calling any script:

```
export MUD_USER=smarty MUD_PASS=goodbyemoon
scripts/mud_start.sh
```

This lands in `data/smarty/player.md` and `data/smarty/world.md` (seeded
from the same templates via `mud_init_memory.sh`) and a `tbamud-smarty` tmux
session, entirely separate from `dummy`'s `tbamud-dummy` session and
`data/dummy/`. Two agents can safely play both characters at the same time
as long as each consistently sets `MUD_USER`/`MUD_PASS` for every script call
in its session.
...
2. **Load persistent memory, once per session, right after starting:**
   `scripts/mud_init_memory.sh`, then Read `data/<user>/player.md` and
   `data/<user>/world.md` (`data/dummy/...` by default, `data/smarty/...`
   when `MUD_USER=smarty`).
...
6. **Update memory as things happen, and again before stopping:**
   Edit `data/<user>/player.md` / `data/<user>/world.md` directly. See
   "Long-term memory" below for what's worth recording.
...
that's what `data/<user>/player.md` and `data/<user>/world.md` are for
(namespaced per character, e.g. `data/dummy/` vs `data/smarty/` -- see
"Playing multiple characters concurrently" above).

- **`data/<user>/player.md`** -- the character's status snapshot, the long-term
